...
export function getFundBullets(fundCode) {
  return request({ url: '/funds/' + encodeURIComponent(fundCode || '') + '/bullet-comments' })
}

export function sendFundBullet(fundCode, data) {
  return request({
    url: '/funds/' + encodeURIComponent(fundCode || '') + '/bullet-comments',
    method: 'POST',
    data
  })
}
...
export function getFundAlertSettings(userId, fundCode) {
  return request({ url: '/users/' + userId + '/fund-alerts/' + encodeURIComponent(fundCode || '') })
}

export function saveFundAlertSettings(userId, fundCode, data) {
  return request({
    url: '/users/' + userId + '/fund-alerts/' + encodeURIComponent(fundCode || ''),
    method: 'PUT',
    data
  })
}
...
export function getProfitAnalysis(userId, options = {}) {
  const params = []
  if (options.accountId !== null && options.accountId !== undefined && options.accountId !== '') {
    params.push('accountId=' + encodeURIComponent(options.accountId))
  }
  if (options.range) params.push('range=' + encodeURIComponent(options.range))
  if (options.calendarMode) params.push('calendarMode=' + encodeURIComponent(options.calendarMode))
  if (options.anchor) params.push('anchor=' + encodeURIComponent(options.anchor))
  if (options.detailKey) params.push('detailKey=' + encodeURIComponent(options.detailKey))
  const query = params.length ? ('?' + params.join('&')) : ''
  return request({ url: '/users/' + userId + '/profit-analysis' + query })
}
